...
public class InspectUpdateBracketLine extends Application {
    ...
    public void start(Stage stage) {
        try {
            ...
            TextPos openPos = TextPos.ofLeading(0, 7); // `{`
            TextPos closePos = TextPos.ofLeading(2, 0); // `}`
            TextPos indentPos = TextPos.ofLeading(0, 0); // `F`

            Rectangle2D indentCaret = getCaretBounds(area, overlay, indentPos, "indent");
            Rectangle2D openCaret = getCaretBounds(area, overlay, openPos, "open");
            Rectangle2D closeCaret = getCaretBounds(area, overlay, closePos, "close");
    ...
    private javafx.geometry.Rectangle2D getCaretBounds(CodeArea editorCodeArea, Pane bracketLineOverlay, TextPos pos, String label) {
                    ...
                    if (layoutInfo != null) {
                        try {
                            ...
                            if (caretInfo != null && caretInfo.getSegmentCount() > 0) {
                                ...
                                System.out.println(label + ": caretInfo=" + caretInfo + " segments=" + (caretInfo!=null ? caretInfo.getSegmentCount() : "null"));
                            }
                        } catch (Exception ex) {
                            System.out.println(label + " Exception in caretInfoAt: " + ex);
                        }
                    } else {
                        System.out.println(label + " layoutInfo is null for para " + targetPara);
                    }
                } else {
                    System.out.println(label + " textFlow is null for para " + targetPara);
                }
                break;
            }
        }
